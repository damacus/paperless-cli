...
from paperless_ngx.core.tags import (
    list_tags, create_tag, delete_tag, get_tag,
)
from paperless_ngx.core.correspondents import (
    list_correspondents, create_correspondent, delete_correspondent,
    get_correspondent,
)
from paperless_ngx.core.doc_types import (
    list_doc_types, create_doc_type, delete_doc_type, get_doc_type,
)
...
def write_config(tmp_path_or_path: str | Path, url: str = BASE_URL,
                 token: str = "testtoken123"):
...
class TestPaperlessBackend:
    ...
    def test_get_success(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/",
                     json={"count": 1, "results": [{"id": 1}]}, status=200)
    ...
    def test_get_401_raises(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/",
                     json={"detail": "auth error"}, status=401)
        backend = make_backend()
    ...
    def test_get_403_raises(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/1/",
    ...
    def test_get_404_raises(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/999/",
    ...
    def test_post_json(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/tags/",
                     json={"id": 5, "name": "test"}, status=201)
        backend = make_backend()
    ...
    def test_post_multipart(self, tmp_path):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/documents/post_document/",
                     json={"task_id": "abc123"}, status=200)
        backend = make_backend()
    ...
    @resp_lib.activate
    def test_patch(self):
        resp_lib.add(resp_lib.PATCH, f"{BASE_URL}/api/documents/1/",
                     json={"id": 1, "title": "Updated"}, status=200)
        backend = make_backend()
    ...
    @resp_lib.activate
    def test_put(self):
        resp_lib.add(resp_lib.PUT, f"{BASE_URL}/api/documents/1/",
                     json={"id": 1, "title": "Replaced"}, status=200)
        backend = make_backend()
    ...
    def test_delete(self):
        resp_lib.add(resp_lib.DELETE, f"{BASE_URL}/api/documents/1/",
                     status=204)
        backend = make_backend()
    ...
    def test_paginate_single_page(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/tags/",
                     json={"count": 2, "next": None, "previous": None,
                           "results": [{"id": 1}, {"id": 2}]}, status=200)
        backend = make_backend()
    ...
    def test_paginate_multiple_pages(self):
        ...
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/tags/",
                     json={"count": 3, "next": page2_url, "previous": None,
                           "results": [{"id": 1}, {"id": 2}]}, status=200)
        resp_lib.add(resp_lib.GET, page2_url,
                     json={"count": 3, "next": None, "previous": None,
                           "results": [{"id": 3}]}, status=200)
        backend = make_backend()
        results = backend.paginate("tags/")
    ...
    def test_paginate_plain_list_response(self):
        ...
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/tasks/",
    ...
    def test_ping_success(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/status/",
                     json={}, status=200)
        backend = make_backend()
        result = backend.ping()
    ...
    def test_ping_connection_error(self):
        import requests
        backend = make_backend()
        with patch.object(backend._session, "get",
    ...
    def test_get_token_success(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/token/",
                     json={"token": "mytoken456"}, status=200)
        backend = make_backend()
    ...
    def test_get_token_failure(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/token/",
                     json={"non_field_errors": ["bad creds"]}, status=400)
        backend = make_backend()
...
class TestDocuments:
    ...
    def test_get_document(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/42/",
                     json={"id": 42, "title": "Test Doc"}, status=200)
        backend = make_backend()
    ...
    def test_upload_document(self, tmp_path):
        ...
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/documents/post_document/",
                     json={"task_id": "uuid-123"}, status=200)
        backend = make_backend()
    ...
    def test_upload_with_tags_and_metadata(self, tmp_path):
        ...
        pdf_file.write_bytes(b"%PDF-1.4 fake")
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/documents/post_document/",
                     json={"task_id": "uuid-456"}, status=200)
        backend = make_backend()
        result = upload_document(
            backend, str(pdf_file), title="Invoice",
            correspondent_id=3, document_type_id=5, tag_ids=[1, 2],
        )
    ...
    def test_download_document_with_content_disposition(self, tmp_path):
        resp_lib.add(
            resp_lib.GET, f"{BASE_URL}/api/documents/1/download/",
    ...
    def test_download_document_fallback_filename(self, tmp_path):
        resp_lib.add(
            resp_lib.GET, f"{BASE_URL}/api/documents/99/download/",
            body=b"content",
    ...
    def test_update_document_title(self):
        resp_lib.add(resp_lib.PATCH, f"{BASE_URL}/api/documents/1/",
                     json={"id": 1, "title": "New Title"}, status=200)
        backend = make_backend()
...
class TestTags:
    ...
    def test_create_tag_custom_color(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/tags/",
                     ...
                     status=201)
        backend = make_backend()
        result = create_tag(backend, "urgent", color="#ff0000")
    ...
    def test_get_tag(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/tags/5/",
                     json={"id": 5, "name": "mytag"}, status=200)
        backend = make_backend()
...
class TestCorrespondents:
    ...
    def test_list_correspondents(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/correspondents/",
    ...
    def test_create_correspondent(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/correspondents/",
                     json={"id": 5, "name": "New Corp"}, status=201)
        backend = make_backend()
    ...
    def test_create_correspondent_with_match(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/correspondents/",
                     json={"id": 6, "name": "ACME"}, status=201)
        backend = make_backend()
    ...
    def test_delete_correspondent(self):
        resp_lib.add(resp_lib.DELETE, f"{BASE_URL}/api/correspondents/5/",
                     status=204)
        backend = make_backend()
    ...
    def test_get_correspondent(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/correspondents/5/",
                     json={"id": 5, "name": "ACME"}, status=200)
        backend = make_backend()
...
class TestDocTypes:
    ...
    def test_list_doc_types(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/document_types/",
                     json={"count": 1, "next": None,
    ...
    def test_create_doc_type(self):
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/document_types/",
                     json={"id": 3, "name": "Contract"}, status=201)
        backend = make_backend()
    ...
    def test_delete_doc_type(self):
        resp_lib.add(resp_lib.DELETE, f"{BASE_URL}/api/document_types/3/",
                     status=204)
        backend = make_backend()
    ...
    def test_get_doc_type(self):
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/document_types/3/",
                     json={"id": 3, "name": "Contract"}, status=200)
        backend = make_backend()
...
class TestSession:
    def test_last_query_set_and_get(self):
        with patch("paperless_ngx.core.session.save_session"):
            with patch("paperless_ngx.core.session.load_session",
                       return_value={}):
                sess = Session()
    ...
    def test_to_dict(self):
        with patch("paperless_ngx.core.session.save_session"):
            with patch("paperless_ngx.core.session.load_session",
                       return_value={"last_query": "foo", "selected_docs": [],
                                     "history": []}):
                sess = Session()
                d = sess.to_dict()
    ...
    def test_persist_is_called_on_mutation(self):
        save_mock = MagicMock()
        with patch("paperless_ngx.core.session.save_session", save_mock):
            with patch("paperless_ngx.core.session.load_session",
                       return_value={}):
                sess = Session()
                sess.last_query = "x"
...
class TestProjectCore:
    ...
    def test_init_connection_with_credentials(self):
        ...
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/token/",
                     json={"token": "acquired_token"}, status=200)
        # ping endpoint
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/status/", json={}, status=200)
        from paperless_ngx.core.project import init_connection
    ...
    def test_get_connection_info(self):
        ...
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/statistics/",
                     json={"documents_total": 42}, status=200)
...
class TestExportCore:
    ...
    def test_bulk_download_all_ok(self, tmp_path):
        for doc_id in [1, 2, 3]:
            resp_lib.add(
                resp_lib.GET, f"{BASE_URL}/api/documents/{doc_id}/download/",
                body=b"PDF content",
    ...
    def test_bulk_download_partial_failure(self, tmp_path):
        ...
        resp_lib.add(
            resp_lib.GET, f"{BASE_URL}/api/documents/2/download/",
            json={"detail": "not found"}, status=404,
        )
        from paperless_ngx.core.export import bulk_download
        backend = make_backend()
    ...
    def test_bulk_download_progress_callback(self, tmp_path):
        ...
        from paperless_ngx.core.export import bulk_download
        backend = make_backend()
        bulk_download(backend, [1], output_dir=str(tmp_path),
...
class TestCLILayer:
    ...
    def test_help_lists_groups(self):
        ...
        assert result.exit_code == 0
        for group in ("document", "tag", "correspondent", "doctype",
                      "project", "export", "status", "repl"):
    ...
    def test_document_help(self):
        ...
        assert result.exit_code == 0
        for cmd in ("list", "get", "upload", "download",
    ...
    def test_document_list_no_config_fails(self):
        ...
        assert result.exit_code != 0
        assert "project init" in result.output or "project init" in (result.stderr or "")
    ...
    def test_document_get_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/42/",
                     json={"id": 42, "title": "Test"}, status=200)
        runner = self._runner()
    ...
    def test_document_search_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/documents/",
                     json={"count": 1, "results": [{"id": 7}]}, status=200)
        runner = self._runner()
    ...
    def test_document_update_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.PATCH, f"{BASE_URL}/api/documents/3/",
                     json={"id": 3, "title": "New"}, status=200)
        runner = self._runner()
    ...
    def test_document_upload_json(self, tmp_path):
        ...
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/documents/post_document/",
                     json={"task_id": "t-1"}, status=200)
        runner = self._runner()
        result = runner.invoke(
            main, ["document", "upload", str(pdf_file), "--json"]
        )
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_tag_create_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/tags/",
                     json={"id": 7, "name": "new-tag"}, status=201)
        runner = self._runner()
    ...
    def test_correspondent_create_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/correspondents/",
                     json={"id": 9, "name": "Corp X"}, status=201)
        runner = self._runner()
        result = runner.invoke(
            main, ["correspondent", "create", "Corp X", "--json"]
        )
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert data["id"] == 9
    ...
    def test_correspondent_delete_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.DELETE, f"{BASE_URL}/api/correspondents/9/",
                     status=204)
        runner = self._runner()
        result = runner.invoke(
    ...
    def test_doctype_create_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.POST, f"{BASE_URL}/api/document_types/",
                     json={"id": 4, "name": "Receipt"}, status=201)
        runner = self._runner()
    ...
    def test_doctype_delete_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.DELETE, f"{BASE_URL}/api/document_types/4/",
                     status=204)
        runner = self._runner()
        result = runner.invoke(
            main, ["doctype", "delete", "4", "--yes", "--json"]
        )
        assert result.exit_code == 0
        data = json.loads(result.output)
        assert data["status"] == "deleted"
    ...
    def test_project_info_json(self):
        save_config(BASE_URL, "tok")
        resp_lib.add(resp_lib.GET, f"{BASE_URL}/api/statistics/",
                     json={"documents_total": 10}, status=200)
        runner = self._runner()
    ...
    def test_export_bulk_zip_honors_original_flag(self, tmp_path):
        ...
        with patch("paperless_ngx.core.export.bulk_download_zip") as mock_zip:
            ...
            result = runner.invoke(
                main,
                ["export", "bulk", "1", "2",
                 "--output-dir", str(tmp_path), "--zip", "--original", "--json"],
            )
